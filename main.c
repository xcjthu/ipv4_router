	...
	while(1)
	{
		...
		if(recvlen>0)
		{
			...
			//	analyseIP(ip_recvpkt);
				...
				// unsigned short iphd = (unsigned short*) ip_recvpkt;

				{
					c = check_sum((unsigned short*) ip_recvpkt, ip_recvpkt->total_len, ip_recvpkt->checksum);
				...
				{
					iphead->checksum = count_check_sum((unsigned short*) ip_recvpkt);
